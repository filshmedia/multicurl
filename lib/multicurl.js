...
multicurl.prototype.getFilesize = function(url, callback) {
  ...
  var filesizeFound = false;
  var options = _.extend({}, this.options, {
    followRedirects: true,
    contentLengthOnly: true,
    destination: null
  });
  ...
  this._connection.on('filesize', function (filesize) {
    filesizeFound = true;
    callback(null, filesize);
  });
  this._connection.on('done', function() {
    if (!filesizeFound) {
      callback(new Error('Failed to find filesize.'), null);
    }
  });
